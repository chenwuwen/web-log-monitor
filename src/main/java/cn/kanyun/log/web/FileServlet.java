...
//            不加utf-8的话会导致中文value乱码
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if (siteRoot == null) {
            handler.handleError(req, resp, "未设置日志文件存储目录,或设置日志文件存储目录错误！");
            return;
        }
...
//            对路径中的特殊字符进行编码
        ...
        File file = new File(siteRoot, URLDecoder.decode(uri.getPath(), "UTF-8"));
        ...
        if (!file.canRead()) {
            ...
            resp.setStatus(403);
            handler.handleError(req, resp, "Local file matched by request URI is not readable");
        ...
        if (file.isFile()) {
//            如果是文件,就处理文件(下载、浏览等)
            handler.nioHandleFile(file, resp);
